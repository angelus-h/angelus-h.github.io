...
```mermaid
graph LR
    A[You're Great at Current Job] --> B[Company Won't Train You for Next Level]
    B --> C[Assumes You'll Learn After Promotion]
    C --> D[Won't Promote Until You Demonstrate Next-Level Skills]
    D --> E[Can't Demonstrate Without Opportunity]
    E --> A
```
...
```mermaid
graph TD
    A[Promotion Spot Opens] --> B{Decision-Makers Ask: Who's Ready?}
    
    B --> C[Look at Performance Data]
    C --> D[Data Says: You're Good at Current Job]
    
    B --> E[Listen to Narrative]
    E --> F{Does Narrative Exist About You?}
    
    F -->|YES| G[Person Comes to Mind - Gets Promoted]
    F -->|NO| H[You're Not in Conversation - Overlooked]
    
    D --> I{But Are They Ready for NEXT Job?}
    I -->|Unknown| H
...
```mermaid
graph TD
    A[Month 1-6: You Excel at Specific Task] --> B[Month 6-12: People Call You the X Person]
    B --> C[Year 1+: Label Sticks in Meetings]
    C --> D[Year 2+: Can't Imagine You Doing Anything Else]
    D --> E[Year 3+: Label Is Permanent]
    
    E --> F{Try to Move to Different Role}
    F --> G[But You're the X Person - Why Change?]
    G --> H[Only Way to Break Label: Leave Company]
...
```mermaid
graph TD
    A[Start: You're Good at Your Job] --> B[Manager Trusts You More]
    B --> C[Team Depends on You]
    C --> D[You Get More Responsibility Same Title]
    
    D --> E{What Happens to Your Time?}
    
    E --> F[Optimize Current Tasks Get Faster]
    F --> G[Stop Learning New Skills]
    
    E --> H[Take on More Work]
    H --> I[Job Becomes 2-3 Jobs]
    
    G --> J[1 Year Later: Great at Current Job]
    I --> J
    
    J --> K[Performance Review: Exceeds Expectations]
    K --> L{Promotion Decision}
    
    L --> M{Decision-Makers Ask: Who's Ready?}
    
    M --> N[Your Manager: They're Great at Current Job]
    N --> O[But No Narrative About Next-Level Readiness]
    
    O --> P[Promotion Goes to Someone Else]
    P --> Q[You're Confused - You Did Everything Right]
    
    Q --> R{Your Response}
    
    R -->|Work Harder at Current Job| A
    R -->|Recognize the Trap| S[Implement Escape Strategies]
